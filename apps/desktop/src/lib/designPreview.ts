...
// iframe/WebView (see designPreview.ts in apps/mobile for the identical
// counterpart — duplicated rather than shared since neither app currently
// depends on a shared workspace package for runtime code).
//
// The returned document embeds a small vanilla-JS editor bridge that:
// - lets the user click any element to select it (outlined, sent to the
...
export type PreviewMessage =
  | { source: "vengaicode-preview"; type: "ready" }
  | { source: "vengaicode-preview"; type: "select" } & PreviewSelection
  | { source: "vengaicode-preview"; type: "deselect" }
  | { source: "vengaicode-preview"; type: "content-changed"; html: string };
...
export function sendEditorCommand(win: Window | null | undefined, command: Record<string, unknown>): void {
  if (!win) return;
  win.postMessage(JSON.stringify({ source: "vengaicode-editor", ...command }), "*");
}
