...
#
#  Supported pairings (SUPPORTED_STACKS below): any of the frontends in
#  DETERMINISTIC_FRONTENDS with any of the backends in
...
#  "_id" for MongoDB) — so a new frontend works with every backend and
#  a new backend with every frontend. Adding one means one more entry in
#  the dispatch dicts near the bottom of this file, not rewriting it.
#
...
def _fields_js(
    ...
    for c in table.columns:
        ...
        if c.fk:
            parent = schema.table(c.fk.ref_table_sql)
            # A MongoDB reference to a row's id holds the document's _id.
            to_document_id = _id_key(backend) == "_id" and c.fk.ref_column == "id"
            value_key = "_id" if to_document_id else c.fk.ref_column
            if to_document_id:
...
// The record id's key: "id" for a SQL backend, "_id" for MongoDB.
const ID_KEY = __ID_KEY__;
__FIELDS__
  ...
  return (
    <div className="p-6">
      ...
      <table className="w-full text-sm border-collapse">
        ...
        <tbody>
          {items.map((item) => (
            <tr key={item[ID_KEY]} className="border-b">
              {FIELDS.map((field) => (
                <td key={field.key} className="p-2">{displayValue(field, item[field.key], options)}</td>
              ))}
              <td className="p-2">
                <button onClick={() => handleDelete(item[ID_KEY])} className="text-red-600 text-xs">Delete</button>
              </td>
            </tr>
          ))}
        </tbody>
...
def _id_key(backend: str) -> str:
    return "_id" if backend in db_schema.MONGOOSE_BACKENDS else "id"
...
def generate_screen_file_react(
    ...
    content = (
        ...
        .replace("__ID_KEY__", js_string_literal(_id_key(backend)))
        .replace("__FIELDS__", "\n".join(_fields_js(table, schema, backend)))
...
<template>
  <div class="p-6">
    ...
    <table class="w-full text-sm border-collapse">
      ...
      <tbody>
        <tr v-for="item in items" :key="item[ID_KEY]" class="border-b">
          <td v-for="field in FIELDS" :key="field.key" class="p-2">{{ displayValue(field, item[field.key], options) }}</td>
          <td class="p-2">
            <button @click="handleDelete(item[ID_KEY])" class="text-red-600 text-xs">Delete</button>
          </td>
        </tr>
      </tbody>
    </table>
...
def build_deterministic_codegen_data(
    ...
    wiring_ctx = WiringCtx(
        ...
        tables=tables,
        # The screens call relative /api URLs; in development Vite forwards
        # them to the backend's own dev port.
        api_proxy_target=f"http://localhost:{knowledge.framework(backend_key).default_port}",
    )
...
def _or_list(keys: list[str]) -> str:
    labels = [_DISPLAY_LABELS.get(k, k) for k in keys]
    return (
        labels[0] if len(labels) == 1 else ", ".join(labels[:-1]) + " or " + labels[-1]
    )


def supported_stacks() -> list[dict]:
    return [
        {"frontend": fe, "backend": be, "api_style": api}
        for fe, be, api in sorted(SUPPORTED_STACKS)
    ]


def supported_stacks_label() -> str:
    """Human-readable list for error messages/UI, e.g.
    'React or Vue with Express or FastAPI (REST)'."""
    frontends = sorted({fe for fe, _be, _api in SUPPORTED_STACKS})
    backends = sorted({be for _fe, be, _api in SUPPORTED_STACKS})
    if len(SUPPORTED_STACKS) == len(frontends) * len(backends):
        return f"{_or_list(frontends)} with {_or_list(backends)} (REST)"
    parts = [
