...
- **GraphQL (2026-09-21): every backend that declares it now has a real builder** — `fastapi` (Strawberry), `flask` (Ariadne), `django` (Graphene-Django), `express` (Apollo Server v5), `spring_boot` (Spring for GraphQL), `rails` (graphql-ruby), `laravel` (Lighthouse — directive-driven, no resolver code needed for CRUD), `actix`/`axum` (async-graphql — compiled and run for real, live mutation+query round-trip confirmed), `gin` (graphql-go/graphql). All 8 frontend adapters (`ScreenCtx.api_style`) generate real GraphQL POST calls instead of REST fetches when paired with one. `nestjs` is the only backend that still declares `graphql` in the matrix without a builder.
- **gRPC: all 3 backends that declare it now have real builders** — `nestjs` (pre-existing), `spring_boot` (net.devh grpc-server-spring-boot-starter — compiled for real, protoc generated real Java stub classes), `aspnet_core` (Grpc.AspNetCore — verified against Microsoft's current docs/NuGet, no live .NET SDK here to compile). No backend still declares `grpc` without a builder.
- **Verification tiers, honestly:** actix/axum/spring_boot(graphql) were compiled and run for real (live HTTP round-trips or, for Spring's GraphQL, its own startup schema-inspection log confirming zero unmapped fields). spring_boot(grpc) was compiled for real (protoc ran, real stub classes produced) but not run live. rails/laravel/gin/aspnet_core(grpc) have no toolchain available in this environment (no Ruby/PHP/Go/.NET SDK) — verified by careful transcription against each library's own current, fetched documentation/source instead of execution.
