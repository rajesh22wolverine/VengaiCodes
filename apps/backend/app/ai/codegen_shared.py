...
NATIVE_CAPABILITY_DESCRIPTIONS: dict[str, str] = {
    ...
    "filesystem": "Filesystem: import { pickFolder, listFiles, scanDevice } from '../native/filesystem'. For a 'scan/find files on my device' user story with NO folder chosen by the user, call await scanDevice(['.mp3', '.flac']) — it automatically checks every drive/mount for common media folders (Music, Documents, Downloads, etc.) and returns every match as { name, path }, with no folder picker needed. For an explicit 'choose a folder' user story, use const folderPath = await pickFolder() (opens the OS folder picker, returns the chosen path or null if cancelled) then await listFiles(folderPath, ['.mp3', '.flac']) to recursively list matches under just that folder. Use either instead of sending a path to a backend — there is no backend at runtime on a packaged desktop/mobile build.",
}
